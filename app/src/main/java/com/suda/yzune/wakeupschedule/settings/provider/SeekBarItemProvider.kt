...
class SeekBarItemProvider : BaseItemProvider<BaseSettingItem>() {
    ...
    override fun convert(holder: BaseViewHolder, data: BaseSettingItem) {
        val item = data as SeekBarItem
        holder.setText(R.id.anko_text_view, item.title)

        if (item.valueInt > item.max || item.valueInt < item.min) {
            holder.setText(R.id.anko_tv_value, "无效值")
            holder.setGone(R.id.anko_tv_unit, true)
            holder.setGone(R.id.anko_tv_prefix, true)
            return
        }

        holder.setText(R.id.anko_tv_value, "${item.valueInt}")
        holder.setGone(R.id.anko_tv_unit, false)
        holder.setGone(R.id.anko_tv_prefix, false)
        holder.setText(R.id.anko_tv_unit, item.unit)

        if (item.prefix.isNotEmpty()) {
            holder.setGone(R.id.anko_tv_prefix, false)
            holder.setText(R.id.anko_tv_prefix, item.prefix)
        } else {
            holder.setGone(R.id.anko_tv_prefix, true)
        }
    }
