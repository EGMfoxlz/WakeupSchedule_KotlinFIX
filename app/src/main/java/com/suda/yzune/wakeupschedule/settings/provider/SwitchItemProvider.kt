...
class SwitchItemProvider : BaseItemProvider<BaseSettingItem>() {
    ...
    override fun convert(holder: BaseViewHolder, data: BaseSettingItem) {
        ...
        holder.getView<AppCompatCheckBox>(R.id.anko_check_box).isChecked = item.checked

        if (item.desc.isEmpty()) {
            holder.setGone(R.id.anko_tv_description, true)
        } else {
            holder.setText(R.id.anko_tv_description, item.desc)
            holder.setGone(R.id.anko_tv_description, false)
        }
    }
