...
class AddCourseAdapter(layoutResId: Int, data: MutableList<CourseEditBean>) :
    ...
    override fun convert(holder: BaseViewHolder, item: CourseEditBean) {
        holder.setText(R.id.et_room, item.room)
        holder.setText(R.id.et_teacher, item.teacher)

        val week = Common.weekIntList2WeekBeanList(item.weekList.value!!).toString()
        holder.setText(R.id.et_weeks, week.substring(1, week.length - 1))

        holder.setText(
            R.id.et_time,
            "${CourseUtils.getDayStr(item.time.value!!.day)}    第${item.time.value!!.startNode} - ${item.time.value!!.endNode}节"
        )
    }
