...
class SchoolImportListAdapter(layoutResId: Int, data: MutableList<SchoolInfo>) :
    ...
    override fun convert(holder: BaseViewHolder, item: SchoolInfo) {
        holder.setGone(R.id.v_line, true)
        holder.setVisible(R.id.tv_top, false)
        holder.setVisible(R.id.tv_bottom, false)
        (holder.getView<View>(R.id.ll_detail).layoutParams as LinearLayoutCompat.LayoutParams)
            .marginEnd = context.dip(16)
        holder.setText(R.id.tv_school, item.name)
        if (item.type == Common.TYPE_LOGIN) {
            holder.setText(R.id.tv_center, "by @${thanksMap[item.name]}")
        } else if (item.sortKey != "通" && item.type != Common.TYPE_HELP) {
            holder.setText(R.id.tv_center, map[item.type])
        } else {
            holder.setText(R.id.tv_center, "")
        }
    }
